...
{
    // Cache TTL en minutos (ej: 60 para 1 hora)
    private $cacheTtl = 60;

    /**
     * Obtiene animales disponibles (no faenados/sacrificados/vendidos)
     * @param bool $forMap Si true, mapea a formato simple
     */
    ...
    public function getLotes(bool $forMap = true)
{
    ...
    if ($forMap) {
        ...
        // Opcional: si no quieres mostrar lotes vacíos
    ...
    /**
     * Obtiene producciones paginadas con filtros
     * @param Request $request Para filtros (ej: tipo, fecha_desde)
     */
    public function getProduccionesPaginated(Request $request)
    {
        $produccionesDiarias = ['leche', 'huevo', 'lana'];

        $query = Produccion::with(['animal.lote'])
            ->whereIn('tipo', $produccionesDiarias)
            ->latest();
    ...
    public function getProduccionStatistics()
    {
        return Cache::remember('produccion_statistics', $this->cacheTtl, function () {
            $produccionesDiarias = ['leche', 'huevo', 'lana'];
            $datos = [];
            foreach ($produccionesDiarias as $tipo) {
                $total = Produccion::where('tipo', $tipo)->sum('valor');
                $datos[$tipo] = [
                    'total' => $total,
                    'meta' => 100,
                    'variacion' => rand(-5, 20), // TODO: Reemplazar con variación real
                    'porcentaje' => min(100, ($total / 100) * 100),
                ];
            }

            // Mejores animales (ranking)
            $mejores = Animal::whereHas('producciones', function ($query) use ($produccionesDiarias) {
                $query->whereIn('tipo', $produccionesDiarias);
            })
            ->withSum(['producciones' => function ($query) use ($produccionesDiarias) {
                $query->whereIn('tipo', $produccionesDiarias);
            }], 'valor')
            ...
            // Tendencias
            $tendencias = Produccion::whereIn('tipo', $produccionesDiarias)
                ->selectRaw('DATE(fecha) as dia, SUM(valor) as total')
                ->groupBy('dia')
                ->orderBy('dia', 'asc')
                ->get();

            // Totales mensuales y variación
            $mesActual = now()->format('Y-m');
            $totalMes = Produccion::whereIn('tipo', $produccionesDiarias)
                ->whereRaw("DATE_FORMAT(fecha, '%Y-%m') = ?", [$mesActual])
                ->sum('valor');

            $mesAnterior = now()->subMonth()->format('Y-m');
            $totalMesAnterior = Produccion::whereIn('tipo', $produccionesDiarias)
            ...
            // Resumen por lotes
            $lotes = Lote::with(['animales.producciones' => function ($query) use ($produccionesDiarias) {
                $query->whereIn('tipo', $produccionesDiarias);
            }])
            ->withCount('animales')
            ->get()
    ...
    /**
     * Calcula ingresos estimados de producción (mejorado con precios reales de ventas)
     */
    public function getProduccionIngresos()
    {
        ...
        // Precio unitario promedio de ventas (mejor que hardcoded)
        $precioUnitarioPromedio = Venta::where('tipo_venta', 'produccion')
            ->whereRaw("DATE_FORMAT(fecha_venta, '%Y-%m') = ?", [now()->format('Y-m')])
            ->avg('precio_unitario') ?? 10; // Fallback a 10 si no hay ventas

        $ingresoMes = $totalMes * $precioUnitarioPromedio;

        // Proyección anual mejorada: Usa promedio de últimos 3 meses
        $promedioMensual = Produccion::whereIn('tipo', ['leche', 'huevo', 'lana'])
    ...
    public function getVentasPaginated(Request $request)
    {
        ...
        // Primero obtener la paginación y luego aplicar el map
    ...
    /**
     * Calcula estadísticas de ventas (con cache, mejoradas)
     */
    public function getVentaStatistics()
    {
        return Cache::remember('venta_statistics', $this->cacheTtl, function () {
            ...
            // Mejoras: Variación mensual, proyección anual
            $mesAnterior = now()->subMonth()->format('Y-m');
            ...
            // Margen de ganancia (asumiendo campo 'costo_total' en Venta - agregar si no existe)
    ...
    /**
     * Calcula inventario de producciones (restando ventas)
     */
    public function getInventarioProducciones()
{
    // Total producido por tipo
    $inventario = [
        'leche' => (float) Produccion::where('tipo', 'leche')->sum('valor'),
        'lana'  => (float) Produccion::where('tipo', 'lana')->sum('valor'),
        'huevo' => (float) Produccion::where('tipo', 'huevo')->sum('valor'),
    ];

    // Ventas: normalizamos "producto" a minúsculas para que coincida con las llaves
    $ventasProducciones = Venta::where('tipo_venta', 'produccion')
        ->where('estado_venta', 'completada')
        ->selectRaw('LOWER(producto) as tipo, SUM(cantidad) as vendido')
        ->groupBy('tipo')
        ->pluck('vendido', 'tipo'); // ['leche' => 100, 'lana' => 50, ...]
...
{
    $inventario = [
        'carne' => Faena::sum('peso_carne'),
        'cuero' => Faena::sum('peso_cuero'),
        'grasa' => Faena::sum('peso_grasa'),
        'plumas' => Faena::sum('peso_plumas'),
        'hueso' => Faena::sum('peso_hueso'),
        'visceras' => Faena::sum('peso_visceras'),
    ];

    // Restar ventas (corregido: suma en SQL y pluck para colección key-value)
    ...
    // Método privado para info de producto en ventas
    ...
    /**
     * Calcula estadísticas de faenas (con cache, mejoradas)
     */
    public function getFaenaStatistics()
    {
        return Cache::remember('faena_statistics', $this->cacheTtl, function () {
            ...
            // Mejoras: Por especie, tendencias
            ...
            // Costos (asumiendo campo 'costo_total' en Faena - agregar si no existe)
    ...
    /**
     * Calcula estadísticas de sacrificios (con cache, mejoradas)
     */
    ...
    // --- Métodos comunes para robustez ---

    /**
     * Valida stock antes de venta/faena/sacrificio
     * @param string $tipo 'produccion' o 'subproducto'
    ...
    /**
     * Calcula ganancia neta global (ingresos ventas - costos faenas/sacrificios/produccion)
     * Asume campos de costos en modelos (agregar si no existen)
     */
    public function calcularGananciaNeta()
    {
        ...
        $costosProduccion = Produccion::sum('costo_total') ?? 0; // Si se agrega costo a producciones

        return $ingresos - ($costosFaenas + $costosSacrificios + $costosProduccion);
    }

    // Agregar más métodos según necesites, ej: exportToCsv($data)
}