...
export default function ModalNuevaCita({ isOpen, onClose, animals = [], vacunas = [] }) {
    ...
    const { data, setData, post, processing, reset, errors } = useForm({
        ...
        responsable:      '',
        estado:           'pendiente',
        // ── Campos de costo (para integración con módulo de Costos) ──
        costo:            '',
        etapa_animal:     'adulta_mantenimiento',
    });
        ...
        <div style={css.backdrop} onClick={handleClose}>
            <div style={css.panel} ref={panelRef} onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={submit} style={css.body}>
                    ...
                    {/* ── COSTO DEL EVENTO ─────────────────────── */}
                    <fieldset style={css.fieldset}>
                        <span style={css.sectionTitle}>Costo del evento</span>
                        <div style={css.row2}>
                            <label style={css.label}>
                                Costo ($) <span style={css.labelHint}>(opcional)</span>
                                <input
                                    type="number"
                                    min="0"
                                    step="0.01"
                            ...
                            <label style={css.label}>
                                Etapa del animal <span style={css.labelHint}>(opcional)</span>
                                <select
                                    value={data.etapa_animal}
                                    onChange={e => setData('etapa_animal', e.target.value)}
                                    style={css.input}
                                >
                                    <option value="lactante">Lactante</option>
                                    <option value="destete">Destete</option>
                                    <option value="desarrollo_crecimiento">Desarrollo / Crecimiento</option>
                                    <option value="primer_empadre_monta">Primer Empadre / Monta</option>
                                    <option value="gestacion">Gestación</option>
                                    <option value="lactacion">Lactación</option>
                                    <option value="periodo_seco">Período Seco</option>
                                    <option value="adulta_mantenimiento">Borrega adulta en mantenimiento</option>
                                </select>
                            </label>
                        </div>
                        {data.costo && parseFloat(data.costo) > 0 && (
                            <p style={{ fontSize: '11px', color: '#059669', marginTop: '4px' }}>
                                ✓ Este gasto se importará automáticamente al módulo de Costos.
                            </p>
                        )}
                    </fieldset>
